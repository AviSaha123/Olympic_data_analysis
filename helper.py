...
def most_succesful(df, sport):
    ...
    x.rename(columns={'index': 'Athletes',
             'Name_x': 'Total Medal Tally', 'region': 'Country'}, inplace=True)
    return x

def year_wise_medals(df, country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    year_df = temp_df[temp_df['region'] == country]
    final_df = year_df.groupby('Year').count()['Medal'].reset_index()
...
def athletes_attributes(df, sport):
